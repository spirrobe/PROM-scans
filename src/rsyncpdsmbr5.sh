...
if [ ! -f "$SYNCFILE" ]; then
  ...
  for scantype in ${scantypes[@]}; do
    echo $scantype
    echo "rsync -avP $dryrun --bwlimit=$bwlimit $device:$remotedir/*$scantype.pd[s,m] $localdir/pds/$scantype/;"
    rsync -avP $dryrun --bwlimit=$bwlimit $device:$remotedir/*$scantype.pd[s,m] $localdir/pds/$scantype/;
  done;
  rm $SYNCFILE
else
